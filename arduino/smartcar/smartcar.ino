...
DirectionalOdometer leftOdometer{
    // for MAC - use : () instead of {} -- windows use {}
    arduinoRuntime, smartcarlib::pins::v2::leftOdometerPins,
    []() { leftOdometer.update(); }, PULSES_PER_METER};
DirectionalOdometer rightOdometer{
    arduinoRuntime, smartcarlib::pins::v2::rightOdometerPins,
    []() { rightOdometer.update(); }, PULSES_PER_METER};
 ...
 * Setup the car:
 * - Activate the serial
 * - Connect the mqtt server (true for local, false for external. Check the
 * Dashboard.java to make sure the app connect to the same server as the car
 * - Takes input from the app
 *
 * For testing, it is recommended using a local host when trying the app and
 * using the serial when testing the car's behaviours. To use the serial,
...
void loop() {
  ...
  if (mqtt.connected()) { // check if the mqtt is connected .. needed if you
...
/**
 * Helper method for MQTTMessageinput() that handles the input from the app
 * received via MQTT.
...
void handleInput() {
  if (Serial.available()) {
    ...
    // TODO: Look at how the mqtt com has been implemented and how it impacts
    // this method
...
/**
 * Brakes in case of emergency. Looks at the direction and reacts to the
 * relevant sensors.
 ...
 */
// TODO for the future: Make sure the situation where leftDirection and
// rightDirection are not equal that it we always want the behaviour described
// in the else part (following)
...
bool reactToSensor(int sensorDistance, int STOP_DISTANCE, bool isSlowDown) {
  if (sensorDistance != 0) { // if the sensor has readings ..
    if (sensorDistance > STOP_DISTANCE && sensorDistance <= 250 && isSlowDown) {
      slowDownSmoothly();
      isParked = true;
    } else if (sensorDistance <=
               STOP_DISTANCE) { // check if the sensor measurement is equal or
                                // less than the stopping distance
...
/**
 * Instructions for the car to perform a "pulling over" maneuver, triggered when
 * last will message is received.
 */
 ...
 * @Returns true if sensor does not detect an obstacle or if it is too close to
 * be detected. False if an obstacle is detected within the range of the sensor.
 */
...
/**
 * Slows down the car smoothly by dividing the speed by 3 until it reaches a
 * safe speed to stop.
 */
...
/**
 * Method to look and compare two sensor values (100ms apart) to know if an
 * obstacle is coming towards the car
 */
void reactToSides() {
  int rightValue = rightIR.getDistance();
  int leftValue = leftIR.getDistance();
...
/**
 * reactToSides() helper method that set the angle of the car at the opposite
 * direction if an obstacle is detected coming towards the car.
 */
 ...
 * example:
 * SR04 front(arduinoRuntime, TRIGGER_PIN, ECHO_PIN, MAX_DISTANCE);  this should
 ...
 * ex:
 * measureDistance (true, "/smartcar/car/distance"); // ex how to use in loop
 * method
 */
...
/**
 * in case of other host just set the IP and the port, local host is false by
 * default.
